...
export const onRequestPost = withErrorHandling(async ({request,env}) => {
  ...
  if(body.action==='enable'){
    ...
    const backup=[]; const statements=[{sql:`UPDATE users SET two_factor_enabled = 1 WHERE id = ?`,params:[user.id]}]; for(let i=0;i<10;i++){const code=Array.from(crypto.getRandomValues(new Uint8Array(8))).map(x=>x.toString(16).padStart(2,'0')).join('').slice(0,10);backup.push(code);statements.push({sql:`INSERT INTO two_factor_backup_codes(user_id,code_hash) VALUES(?,?)`,params:[user.id,await hashToken(code)]});} await db.batch(statements); return ok({enabled:true,backup_codes:backup},'ورود دومرحله‌ای فعال شد؛ کدهای پشتیبان را فقط همین یک‌بار ذخیره کنید');
  }
