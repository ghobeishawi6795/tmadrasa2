import {q} from "../_shared/db.js";import {authenticate,requireRole} from "../_shared/auth.js";import {ok,errors} from "../_shared/response.js";import {readJson,requireFields,withErrorHandling} from "../_shared/validate.js";
...
await db.run(`UPDATE schools SET plan=COALESCE(?,plan),subscription_status=COALESCE(?,subscription_status),subscription_expires_at=COALESCE(?,subscription_expires_at),trial_ends_at=COALESCE(?,trial_ends_at),active=${activeSql} WHERE id=?`,b.plan||null,b.status||null,b.expires_at||null,b.trial_ends_at||null,b.school_id);return ok(null,'اشتراک بروزرسانی شد');});
