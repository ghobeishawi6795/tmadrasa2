...
export const onRequestGet = withErrorHandling(async ({ request, env }) => {
    ...
    const unread = await db.first(
        `SELECT COUNT(*) as c FROM notifications WHERE user_id = ? AND school_id = ? AND read_at IS NULL`, user.id, user.school_id
    );
...
export const onRequestPost = withErrorHandling(async ({ request, env }) => {
    ...
    if (body.action === "mark_read") {
        ...
        const notif = await db.first(`SELECT * FROM notifications WHERE id = ? AND school_id = ?`, body.id, user.school_id);
        if (!notif) throw errors.notFound("اعلان پیدا نشد");
        if (notif.user_id !== user.id) throw errors.forbidden("این اعلان متعلق به شما نیست");
        await db.run(`UPDATE notifications SET read_at = datetime('now') WHERE id = ? AND school_id = ?`, notif.id, user.school_id);
        return ok(null, "اعلان خوانده شد");
    }

    if (body.action === "mark_all_read") {
        await db.run(
            `UPDATE notifications SET read_at = datetime('now') WHERE user_id = ? AND school_id = ? AND read_at IS NULL`,
            user.id, user.school_id
        );
